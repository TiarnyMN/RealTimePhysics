...
{
	CreateShaderFromFile("Shaders\\" + shaderVertexPath, "Shaders\\" + shaderFragmentPath);
}
...
bool Shader::CreateShaderFromFile(const string &shaderVertexPath, const string& shaderFragmentPath)
{
	...
	{
		fprintf(stderr, "ERROR: Shader file cannot be loaded - %d '%s'\n", shaderVertexPath);
        exit(0);
	}
	...
	if (Success == 0) 
	{
		...
		fprintf(stderr, "ERROR: Unable to link shader program - %d '%s'\n", ErrorLog);
        exit(1);
	}
	...
	{
        glGetProgramInfoLog(shaderID, sizeof(ErrorLog), NULL, ErrorLog);
        fprintf(stderr, "ERROR: Shader program is invalid - %d '%s'\n", ErrorLog);
        exit(1);
    }
...
bool Shader::AddShader(GLuint ShaderProgram, const char* pShaderText, GLenum ShaderType)
{
    ...
    if (ShaderObj == 0)
	{
        fprintf(stderr, "ERROR: Unable to create shader type - %d '%s'\n", ShaderType);
        exit(0);
    }
    ...
    if (!success) 
	{
        ...
        fprintf(stderr, "ERROR: Unable to compile shader type - %d '%s'\n", ShaderType, InfoLog);
        exit(1);
    }
